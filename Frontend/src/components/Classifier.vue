  ...
  <section class="container mx-auto px-4 py-16 bg-[var(--chat-bg)]">
    <div class="mx-auto max-w-full md:max-w-6xl">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div
          class="rounded-lg border p-6 shadow-md transition-shadow duration-300 hover:shadow-lg bg-[var(--chat-card)] border-[var(--chat-text)] hover:border-blue-500">
          ...
          <div class="space-y-4 pt-6">
            <textarea placeholder="Cole ou digite seu texto aqui para classificá-lo..." v-model="text"
              ...
              class="w-full min-h-[180px] resize-none rounded-md border p-3 bg-[var(--chat-card)] text-[var(--chat-text)] focus:border-[var(--chat-accent] focus:ring-2 focus:ring-[var(--chat-accent)] transition-colors duration-300  hover:border-blue-500"></textarea>
            <div v-if="!attachedFileName"
              ...
              <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 text-[var(--chat-text)]" viewBox="0 0 24 24"
            ...
            <div v-if="attachedFileName"
              ...
              <button @click="removeFile" class="text-red-500 hover:text-red-700 transition-colors duration-200"
                :class="{ 'cursor-pointer': attachedFileName }" aria-label="Remover anexo">
            ...
            <button @click="classifyTextBackend" :disabled="(!text.trim() && !attachedFile) || isLoading" class="w-full rounded-md py-3 text-lg font-semibold transition-colors duration-300
         bg-[var(--chat-accent)] text-white hover:bg-[var(--chat-accent)]/90 disabled:bg-gray-400"
              :class="{ 'cursor-pointer': (text.trim() || attachedFile) && !isLoading }">
              {{ isLoading ? 'Gerando...' : 'Classificar Texto' }}
            </button>
        ...
        <div
          ...
          <div class="space-y-6 pt-6">
            <div v-if="result">
              ...
              <div
                class="rounded-lg p-4 border bg-[var(--chat-card)] border-[var(--chat-text)] relative hover:border-blue-500 transition-colors duration-200 flex flex-col">
                <h4 class="mb-2 font-semibold text-[var(--chat-text)]">Sugestão de Resposta / Ação</h4>
                <button @click="copySuggestion"
                  class="absolute top-4 right-4 text-[var(--chat-text)] hover:text-[var(--chat-accent)] cursor-pointer"
                ...
                <button @click="regenerateSuggestion" :disabled="isSuggestionLoading"
                  class="mt-8 w-full rounded-md py-2 text-sm font-semibold transition-colors duration-300
                         bg-[var(--chat-accent)] text-white hover:bg-[var(--chat-accent)]/90 disabled:bg-gray-400 cursor-pointer" :class="{ 'cursor-pointer': !isSuggestionLoading }"
